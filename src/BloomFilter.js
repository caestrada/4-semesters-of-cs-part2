...
class BloomFilter {
    _cache = new Array(100).fill(0);

    add(string) {
        this._cache[h1(string)] = 1;
        this._cache[h2(string)] = 1;
        this._cache[h3(string)] = 1;
    }

    contains(string) {
        return !!(this._cache[h1(string)] && this._cache[h2(string)] && this._cache[h3(string)]);
    }
};
